...
if __name__ == "__main__":
    ...
    labeled_dataset = VerifyData(
        tokenizer, args.labeled_data_path, "train", short=args.short
    )
    valid_dataset = VerifyData(
        tokenizer, args.valid_data_path, "valid", short=args.short
    )
    ...
    if args.only_labeling == 0:
        teacher_trainer.work(
            train_data=labeled_dataset, test=True, save=True, train=True
        )
        save_test_result(
            args.test_data_path,
            teacher_trainer.test_result_dict,
